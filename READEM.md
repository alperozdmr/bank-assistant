...
# SSS sistemi için gerekli paketleri yükleyin
pip install sentence-transformers==2.2.2 faiss-cpu==1.7.4 numpy==1.24.33 

2. SSS Verisini Kontrol Edin
SSS verileri backend/data/faq.json dosyasında bulunmaktadır. Sistem 100+ hazır soru-cevap ile gelir.
# SSS dosyasının varlığını kontrol edin
ls backend/data/faq.json

Kullanım
SSS Sistemini Test Etme
cd backend

# 1. FAQTool'u direkt test edin
python -m tools.faq_tool

# 2. Agent üzerinden test edin
python -m agent.agent
# Örnek sorular yazın:
# > Mobil bankacılık şifremi unuttum
# > Para transferi nasıl yapılır?
# > Kredi başvurusu yapabilir miyim?

Konfigürasyon
Benzerlik Eşiği Ayarlama
backend/agent/agent.py dosyasında threshold değerlerini ayarlayabilirsiniz:
# Varsayılan değerler
faq_results = faq_tool.search(user_text, top_k=3, threshold=0.65)  # Arama hassasiyeti
if faq_results and faq_results[0]['similarity_score'] >= 0.75:     # Cevap verme eşiği
threshold: 0.60-0.70 arası geniş eşleşme, 0.80+ kesin eşleşme
...
Import Hatası
# Paketlerin doğru yüklendiğinden emin olun
pip list | grep -E "sentence-transformers|faiss|numpy"

Huggingface Hub Hatası
# huggingface-hub paketini güncelleyin
pip install --upgrade huggingface-hub

Performans Metrikleri

İlk Yükleme: ~2-3 saniye (model yükleme)
Arama Hızı: 10-50ms per sorgu
Bellek Kullanımı: ~200MB (model + index)
Doğruluk: %85+ (benzer sorular için)

Teknik Detaylar

Embedding Modeli: paraphrase-multilingual-MiniLM-L12-v2 (Türkçe destekli)
Vektör Boyutu: 384 dimension
Arama Algoritması: FAISS L2 distance + normalization
Güven Seviyeleri: high (>0.9), medium (0.8-0.9), low (<0.8)