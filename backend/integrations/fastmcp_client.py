...
def _as_dict(x: Any) -> Dict[str, Any]:
    ...
    if isinstance(x, str):
        try:
            ...
            pass
    # debug fallback - _raw içindeki JSON'u parse etmeye çalış
    raw_str = str(x)
    if "TextContent" in raw_str and "text=" in raw_str:
        # TextContent(type='text', text='{"ok": true...}') formatını parse et
        try:
            import re
            match = re.search(r"text='([^']+)'", raw_str)
            if match:
    ...
    return {"_type": type(x).__name__, "_raw": raw_str}
