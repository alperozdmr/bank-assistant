...
try:
    from config_local import LLM_API_BASE as _BASE
    from config_local import LLM_API_KEY as _KEY
    from config_local import LLM_CHAT_PATH as _CHAT_PATH
    from config_local import LLM_MODEL as _MODEL
except Exception:
    _BASE = "https://router.huggingface.co/v1"  # Hugging Face Router varsayılanı
    _CHAT_PATH = "/chat/completions"  # Hugging Face API yolu
    _MODEL = "qwen/qwen3-30b-a3b:free"  # Hugging Face modeli
    _KEY = "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"  # HF kullanıyorsan değiştir
...
class LLMAdapter:
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        if not self.api_key or self.api_key == "REPLACE_ME":
            raise RuntimeError(
                "LLM_API_KEY eksik: HF/benzeri için anahtar verin veya Ollama kullanın."
            )
        h["Authorization"] = f"Bearer {self.api_key}"
        return h

    def system_messages(
        self, extra_system: Optional[str] = None
    ) -> List[Dict[str, str]]:
    ...
    def generate(
        ...
        try:
            ...
            return {
                ...
                "detail": r.text if "r" in locals() else "",
